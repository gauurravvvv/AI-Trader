#!/bin/bash
# Initialize all FinAgent Pool servers for natural language backtesting (no conda version)
...
start_agent_pool() {
    ...
    # Launch service using system/python
...
start_mcp_server() {
    ...
    # Launch MCP server using system/python
...
start_a2a_server() {
    ...
    # Launch A2A server using system/python
    nohup python a2a_server.py > ${log_file} 2>&1 &
    local pid=$!
    echo ${pid} > ${pid_file}
